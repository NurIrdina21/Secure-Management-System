...
function suspendUserAccount($userId) {
    global $con;
    $stmt = $con->prepare("UPDATE employee SET is_suspended = 1 WHERE user_id = ?");
...
function incrementFailedLoginAttempts($userId) {
    global $con;
    $maxFailedAttempts = 3;
    $stmt = $con->prepare("UPDATE employee SET failed_login_attempts = failed_login_attempts + 1 WHERE user_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function getUserDataByUserId($userId) {
    global $con;
    $stmt = $con->prepare("SELECT * FROM employee WHERE user_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
    ...
    if(!empty($surname) && !empty($password))
    {
        ...
        if($result && mysqli_num_rows($result) > 0)
        {
            ...
            if($password === $decrypted_password)
            {
                ...
                $stmt = $con->prepare("UPDATE employee SET failed_login_attempts = 0 WHERE surname = ?");
