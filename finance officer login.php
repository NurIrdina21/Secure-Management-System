...
function suspendUserAccount($userId) {
    global $con;
    $stmt = $con->prepare("UPDATE fo SET is_suspended = 1 WHERE user_id = ?");
...
function incrementFailedLoginAttempts($userId) {
    global $con;
    $maxFailedAttempts = 3;
    $stmt = $con->prepare("UPDATE fo SET failed_login_attempts = failed_login_attempts + 1 WHERE user_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
    ...
    if ($user_data['failed_login_attempts'] >= $maxFailedAttempts) {
        ...
        header("Location: finance officer login.php");
        exit;
    }
}
...
function getUserDataByUserId($userId) {
    global $con;
    $stmt = $con->prepare("SELECT * FROM fo WHERE user_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(!empty($surname) && !empty($password))
    {
        ...
        $query = "SELECT * FROM fo WHERE surname = '$surname' LIMIT 1";
