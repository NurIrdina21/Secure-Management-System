...
function suspendUserAccount($userId) {
    global $con;
    $stmt = $con->prepare("UPDATE pm SET is_suspended = 1 WHERE user_id = ?");
...
function incrementFailedLoginAttempts($userId) {
    global $con;
    $maxFailedAttempts = 3;
    $stmt = $con->prepare("UPDATE pm SET failed_login_attempts = failed_login_attempts + 1 WHERE user_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
function getUserDataByUserId($userId) {
    global $con;
    $stmt = $con->prepare("SELECT * FROM pm WHERE user_id = ?");
    $stmt->bind_param("i", $userId);
    $stmt->execute();
...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(!empty($surname) && !empty($password))
    {
        ...
        $query = "SELECT * FROM pm WHERE surname = '$surname' LIMIT 1";
